...
int GetAppConfigPath(char *path, size_t size, const char *name)
{
	...
	if (portable_mode) {
#ifdef __APPLE__
		const string relativePath = name && *name ? "../config/"s + name : "../config";
		BPtr<char> portablePath = os_get_executable_path_ptr(relativePath.c_str());
		return portablePath ? snprintf(path, size, "%s", portablePath.Get()) : -1;
#else
		return name && *name ? snprintf(path, size, CONFIG_PATH "/%s", name)
				    : snprintf(path, size, CONFIG_PATH);
#endif
	} else {
...
#ifdef __APPLE__
		const string relativePath = name && *name ? "../config/"s + name : "../config";
		return os_get_executable_path_ptr(relativePath.c_str());
#else
		char path[512];
		const int length = name && *name ? snprintf(path, sizeof(path), CONFIG_PATH "/%s", name)
						: snprintf(path, sizeof(path), CONFIG_PATH);
		return length > 0 ? bstrdup(path) : nullptr;
#endif
	} else {
