...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, "split-capture/logs", lastLogFile);

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << "split-capture/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file(false, "split-capture/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("split-capture/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, "split-capture/profiler_data");
...
static void main_crash_handler(const char *format, va_list args, void * /* param */)
{
	...
	string crashFilePath = "split-capture/crashes";
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		...
		static constexpr char dualCaptureFailpointPrefix[] = "--dual-capture-failpoint=";
		if (strncmp(argv[i], dualCaptureFailpointPrefix, sizeof(dualCaptureFailpointPrefix) - 1) == 0) {
			const char *name = argv[i] + sizeof(dualCaptureFailpointPrefix) - 1;
			if (!DualCaptureRecorder::SetTestFailpoint(name)) {
				cerr << "Invalid Dual Capture failpoint '" << name
				     << "'. Expected preflight, desktop-start, or camera-start." << endl;
				return 2;
			}
			continue;
		}
		...
		if (arg_is(argv[i], "--multi", "-m")) {
			...
			std::cout << "Split Capture - " << App()->GetVersionString(false) << "\n";
			exit(0);
		}
	}

#if ALLOW_PORTABLE_MODE
	auto portableMarkerExists = [](const char *name) {
#ifdef __APPLE__
		const string relativePath = "../"s + name;
#else
		const string relativePath = BASE_PATH "/"s + name;
#endif
		BPtr<char> path = os_get_executable_path_ptr(relativePath.c_str());
		return path && os_file_exists(path);
	};
	if (!portable_mode) {
		portable_mode = portableMarkerExists("portable_mode") ||
				portableMarkerExists("obs_portable_mode") ||
				portableMarkerExists("portable_mode.txt") ||
				portableMarkerExists("obs_portable_mode.txt");
	}

	if (!opt_disable_updater) {
		opt_disable_updater =
			portableMarkerExists("disable_updater") || portableMarkerExists("disable_updater.txt");
	}

	if (!opt_disable_missing_files_check) {
		opt_disable_missing_files_check = portableMarkerExists("disable_missing_files_check") ||
						  portableMarkerExists("disable_missing_files_check.txt");
	}
#endif
