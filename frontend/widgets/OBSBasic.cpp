...
OBSBasic::OBSBasic(QWidget *parent) : OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	auto *splitSceneRecording = new QAction(QTStr("DualCapture.AdvancedReturn"), this);
	const auto toolsActions = ui->menuTools->actions();
	ui->menuTools->insertAction(toolsActions.isEmpty() ? nullptr : toolsActions.constLast(), splitSceneRecording);
	connect(splitSceneRecording, &QAction::triggered, this, [this] {
		if (!dualCaptureDashboard)
			dualCaptureDashboard = new DualCaptureDashboard(this);
		dualCaptureDashboard->OpenFocused();
	});
...
#undef SET_VISIBILITY
	...
	if (!dualCaptureDashboard)
		dualCaptureDashboard = new DualCaptureDashboard(this);
	QTimer::singleShot(0, dualCaptureDashboard, [this] { dualCaptureDashboard->OpenFocused(); });
}
