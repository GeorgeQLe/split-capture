...
try {
	...
	if (FetchAndVerifyFile("branches", "split-capture\\updates\\branches.json", WIN_BRANCHES_URL, &text)) {
	...
	text.clear();
	if (!FetchAndVerifyFile("manifest", "split-capture\\updates\\manifest.json", manifestUrl.c_str(), &text,
				extraHeaders)) {
		return;
	}
	...
	if (!FetchAndVerifyFile("updater", "split-capture\\updates\\split-capture-updater.exe", WIN_UPDATER_URL,
				nullptr)) {
		return;
	}
	...
	BPtr<char> updateFilePath =
		GetAppConfigPathPtr("split-capture\\updates\\split-capture-updater.exe");
